...
namespace PSGM.Lib.ExifData
{
    ...
    public class ExifData
    {
        ...
        public bool GetTagValue(ExifTag TagSpec, out string Value, StringCoding Coding)
        {
            TagItem t;
            bool Success = false;
            int i, j;

            StringCodingFormat StringTagFormat = (StringCodingFormat)((uint)Coding & 0xFFFF0000);
            ushort CodePage = (ushort)Coding;
            if (StringTagFormat == StringCodingFormat.TypeUndefinedWithIdCode)
            {
            ...
            else
            {
                ...
                if (GetTagItem(TagSpec, out t))
                {
                    ...
                    if (CodePage != 0)
                    {
                        if (((StringTagFormat == StringCodingFormat.TypeAscii) && (t.TagType == ExifTagType.Ascii)) ||
                            ((StringTagFormat == StringCodingFormat.TypeUndefined) && (t.TagType == ExifTagType.Undefined)) ||
                            ((StringTagFormat == StringCodingFormat.TypeByte) && (t.TagType == ExifTagType.Byte)))
                        {
        ...
        public bool SetTagValue(ExifTag TagSpec, string Value, StringCoding Coding)
        {
            ...
            {
                if (StringTagFormat == StringCodingFormat.TypeUndefined)
                {
                    TagType = ExifTagType.Undefined;
                }
                else if (StringTagFormat == StringCodingFormat.TypeByte)
                {
        ...
        private static readonly byte[] JpegExifSignature = new byte[]
        {
            (byte)'E',
            (byte)'x',
            (byte)'i',
        ...
        private static readonly byte[] JpegXmpInitialSignature = new byte[]
        {
            (byte)'h',
            (byte)'t',
            (byte)'t',
        ...
        private static readonly byte[] JpegIptcSignature = new byte[]
        {
            (byte)'P',
            (byte)'h',
            (byte)'o',
        ...
        private static readonly byte[] PngXmpSignature = new byte[]
        {
            (byte)'X',
            (byte)'M',
            (byte)'L',
        ...
        private static readonly byte[] PngIptcSignature = new byte[]
        {
            (byte)'R',
            (byte)'a',
            (byte)'w',
        ...
        private static readonly uint[] Crc32ChecksumTable = new uint[]
        {
        ...
        private bool GetGpsCoordinateHelper(out GeoCoordinate Value, ExifTag ValueTag, ExifTag RefTag, char Cp1, char Cp2)
        {
            ...
            if (GetTagValue(ValueTag, out Deg, 0) && Deg.IsValid() &&
                ...
                GetTagValue(RefTag, out Ref, StringCoding.Utf8) && (Ref.Length == 1))
            {
                CardinalPoint = Ref[0];
        ...
        private bool SetGpsCoordinateHelper(GeoCoordinate Value, ExifTag ValueTag, ExifTag RefTag, char Cp1, char Cp2)
        {
            ...
            if (SetTagValue(ValueTag, Deg, ExifTagType.URational, 0) &&
                ...
                SetTagValue(RefTag, Value.CardinalPoint.ToString(), StringCoding.Utf8))
            {
                Success = true;
            }
            return (Success);
        ...
        private bool GetDateAndTimeWithMillisecHelper(out DateTime Value, ExifTag DateAndTimeTag, ExifTag MillisecTag)
        {
            ...
            {
                Success = true;
                if (GetTagValue(MillisecTag, out string SubSec, StringCoding.Utf8))
                {
                    string s = SubSec;
                    int len = s.Length;
        ...
        private bool SetDateAndTimeWithMillisecHelper(DateTime Value, ExifTag DateAndTimeTag, ExifTag MillisecTag)
        {
            ...
            if (SetTagValue(DateAndTimeTag, Value))
            {
                ...
                if ((MilliSec != 0) || TagExists(MillisecTag))
                {
                    ...
                    Success = Success && SetTagValue(MillisecTag, s, StringCoding.Utf8);
                }
            }
            return (Success);
        }
