    ...
    public partial class UISplashScreen : Window
    {
        ...
        private void InitializeVariables()
        {
            ...
            if (Globals.Device == null)
            {
                ...
                {
                    PowerSupply = new Globals_Device.Globals_Device_PowerSupply()
                    {
                        Nextys = null
                    }
                };
            ...
            int devicesCount = Globals.Context.Devices.Where(i => i.DeviceManufacturer == DeviceManufacturers.Nextys).ToList().Count();
            if (devicesCount > 0)
            {
                Globals.Device.PowerSupply.Nextys = new Lib.PowerSupply.Nextys_Container();
            }
            else
            {
            ...
            if (Globals.Device.PowerSupply != null)
            {
                if (Globals.Device.PowerSupply.Nextys != null)
                {
                    _nextys = Globals.Device.PowerSupply.Nextys;
                }
            }
            #endregion
        }
        ...
        private void PowerSupplyInitializeAndConnect()
        {
            ...
            {
                List<Device> devices = Globals.Context.Devices.Where(i => i.DeviceManufacturer == DeviceManufacturers.Nextys && i.DeviceType == DeviceTypes.PowerSupply)
                ...
                foreach (Device device in devices)
                {
                    try
                    {
                        ...
                        if (device.InitialzeAtSplashscreen)
                        {
                            ...
                            Interface_Serial? serial = device.Interfaces_Serial;
                            _nextys.DcDcConverters.Add(new Nextys_DcDcConverter(serial.PortName, serial.BaudRate, (Parity)serial.Parity, (StopBits)serial.StopBits, (Handshake)serial.Handshake, serial.ReadTimeout, serial.WriteTimeout, serial.MonitoringInterval, 0x01));
