...
{
    public class DbStorageStructure_Context : DbContext
    {
        #region Variables
        ...
        private string _connectionStringSQLite = "Data Source=C:\\Git\\MSD\\Robot\\80_Model\\PSGM.Model.DbStorageStructure\\DbStorageStructure.db";
        ...
        #region Context
        public DbStorageStructure_Context() : base()
        {
        }

        public DbStorageStructure_Context(DbContextOptions<DbStorageStructure_Context> options) : base(options)
        {
        }

        public DbStorageStructure_Context(DatabaseType databaseType, string connectionString) : base()
        {
        ...
        public override int SaveChanges()
        {
            ...
            foreach (var entry in entries)
            {
                switch (entry.Entity)
                {
                    case DbStorageStructure_Project project:
                        Project_AuditLog.Add(new DbStorageStructure_Project_AuditLog
                        {
                            Id = new Guid(),
                    ...
                    case DbStorageStructure_ProjectStorage projectStorage:
                        ProjectStorage_AuditLog.Add(new DbStorageStructure_ProjectStorage_AuditLog
                        {
                            ...
                            SourceId = projectStorage.Id,
                            //TableName = entry.Metadata.GetTableName(),
                            //EntityName = projectStorage.GetType().Name,
                            Action = entry.State.ToString(),
                            DateTime = DateTime.UtcNow,
                            Changes = JsonConvert.SerializeObject(entry.CurrentValues.ToObject())
                        });
